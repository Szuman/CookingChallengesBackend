...
class AuthorizationControllerTest {
    ...
    void postComment() throws Exception {
        ...
        when(commentApiFacade.postComment(postComment)).thenReturn(id);
        ...
        verify(commentApiFacade).postComment(postComment);
    }
    ...
    void getByContentId() throws Exception {
        ...
        when(commentApiFacade.findByContentId(contentId)).thenReturn(List.of(commentDTO));

        //expect
        this.mockMvc.perform(get("/comments/content/" + contentId))
                .andExpect(status().isOk())
                .andExpect(content().string(objectMapper.writeValueAsString(commentDTO)));
        verify(commentApiFacade).findByContentId(contentId);
    }
    ...
    void getByUserId() throws Exception {
        ...
        when(commentApiFacade.findByUserId(userId)).thenReturn(List.of(commentDTO));

        //expect
        this.mockMvc.perform(get("/comments/user/" + userId))
                .andExpect(status().isOk())
                .andExpect(content().string(objectMapper.writeValueAsString(commentDTO)));
        verify(commentApiFacade).findByContentId(userId);
    }
}
